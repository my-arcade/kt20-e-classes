...
export default class Parser {
  ...
  async getFromRemote() {
    ...
    const {data: {accessToken: token}} = await axios.post(this.loginUrl, {email: this.email, password: this.password})

    this.log(`${chalk.blue('[Info]')} Login successful`)
    this.log(`${chalk.blue('[Info]')} Requesting remote api docs...`)
    const {data: {paths, definitions}} = await axios.get(this.baseUrl, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
    this.log(`${chalk.blue('[Info]')} Started parsing...`)
    this.paths = Object.getOwnPropertyNames(paths)
    .map(key => ({
      path: key,
  ...
  filter(urlRegex: string) {
    return this.paths
    .filter(path => path.path.indexOf(urlRegex) === 0)
  }

  _getMethods(paths: string, key: string) {
    return Object.getOwnPropertyNames(paths[key]).map(method => ({...paths[key][method], method}))
  }
}
